...
export default function SurplusForecastCard() {
  ...
  return (
    <div className="bg-white rounded-3xl shadow-sm p-6 sm:p-8 border border-slate-100 space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b pb-4">
        <div>
          <h2 className="text-xl sm:text-2xl font-extrabold text-slate-900 flex items-center gap-2">
            🔮 {t("cards.surplus_forecast")}
          </h2>
          <p className="text-xs text-slate-500 mt-1 font-medium">
            {t("cards.surplus_subtitle")}
          </p>
        </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-7 gap-2 sm:gap-3 items-end h-44 pt-4 pb-2 border-b border-slate-100">
          {forecastData.map((item, idx) => (
            <div key={idx} className="flex flex-col items-center gap-2 h-full justify-end group">
              <span className="text-[10px] sm:text-[11px] font-extrabold text-slate-700">{item.surplusKg}kg</span>
              <div
              ...
              ></div>
              <span className="text-[10px] sm:text-xs font-bold text-slate-600 mt-1">{item.day}</span>
            </div>
          ))}
        </div>
      )}
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between text-xs text-slate-600 bg-purple-50/80 p-4 rounded-2xl border border-purple-100 gap-3">
        <span className="font-semibold text-purple-950">
          💡 <strong className="text-purple-900">{t("ml.prediction")}:</strong> {recommendation}
        </span>
        <button
